...
net.Receive("vj_fnafsb_gm_dat",function(len,ply)
	...
	for _,v in pairs(classes) do
		...
		table.insert(self.Classes,{Class = v,Spotted = false})
	end
end)
...
function ENT:Think()
	for _,v in pairs(player.GetAll()) do
		...
		if !found then
			...
			self.PlayerMusic[index] = {Player = v,Music = nil,Tracks = fixedFiles,NextTrackT = 0}
			-- local song = VJ_PICK(self.PlayerMusic[index].Tracks)
			...
			-- end)
			-- print("Started " .. song)
			-- self.PlayerMusic[index].NextTrackT = CurTime() +VJ_SoundDuration(song)
		else
			local index = 0
			...
			if CurTime() > tbl.NextTrackT then
